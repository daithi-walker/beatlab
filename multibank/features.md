...
- **Export** — Export ▾ dropdown with WAV, MP3, MIDI, and Stems (ZIP), via shared `core/export.js`. `initAudio()` was refactored into a shared `buildEngine(context)` so live and offline graphs are built by identical code. `renderMulti()` renders through an `OfflineAudioContext`, swapping the module audio globals and the per-panel `nodes` map (rebuilding each panel's full FX chain) for the synchronous scheduling pass, restored before `startRendering()`. MIDI puts drums on GM channel 10 and each melodic panel on its own channel with its note (sub down an octave). Stems render each non-empty panel in isolation. MP3 uses vendored `lamejs` (only dependency; see `docs/adr-008-lamejs-mp3.md`); WAV/MIDI/stems are dependency-free.
